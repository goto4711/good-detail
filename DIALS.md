...
| `build_training_data.py` (data harness) | `candidates.json --label reward` (bootstrap now) or `--label human --responses responses/ --min_gap N` (gold). Emits `sft_<label>.jsonl` + `preference_pairs_<label>.jsonl` → feed `sft_narrative.py --source jsonl --data …` and `dpo_kto_train.py --pairs …`. Dials: `--gate_unsup`, `--fab_unsup`, `--fab_F`, `--gate_human_chosen`, `--dry_run` |
