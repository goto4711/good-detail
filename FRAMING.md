...
*reasoning* model available to us, gpt-oss-120b, would not reliably produce a clean
micro-narrative at all, defaulting to verbose reasoning over content — the same format
pathology we documented for it on classification. (A standard *instruct* model of the
same provenance, mistral-small-3.2, writes cleanly — confirming the bottleneck was the
model *kind*, not raw scale, and that on clean prose the rewards still demonstrably
disagree, 5/5 records.) The lever that moved grounding was not scale but **focused
source passages and a defined target**: grounding generations against retrieved source
sentences roughly halved fabrication versus an entity-list record (unsupported
specifics 6.7 → 4.2). Scale was not the
...
## The lesson, sharpened — data and the inverse

For a contested, generative cultural target, the bitter lesson is not merely
occasional; it **inverts**. Two things together produce the inversion. First, it is a
*data* result: the binding constraint was never compute or model size but **quality,
situated, human-labelled data** — the one input pre-training cannot supply for a
target like "good detail," because the relevant signal is local, distributional, and
contested rather than culturally general. Second, with that constraint binding, the
ordering Sutton describes runs backwards: **expert/situated knowledge out-performs
scale**, and the more you scale a general method against a contested target the more
it homogenises, hallucinates, or collapses (our GRPO arms, the saturated judge, the
record-dumping surface reward). So the right reading is not "the lesson sometimes
fails" but "for this class of problem the lesson is **inverted** — data quality and
theory dominate scale." One cannot let computation discover "good detail," because the
target must first be theorised and community-defined before any model can be judged
against it. This is the earlier
